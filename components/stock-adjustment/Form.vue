<template>
  <el-row :gutter="20">
    ...
    <el-col :md="8">
      <el-form-item
        :label="$t('columns.adjustment_date')"
        prop="adjustmentDate"
        :rules="[{ required: true }]"
      >
        <el-date-picker 
          v-model="formRef.adjustmentDate"
          type="date"
...
import { CrudDialogState } from '~/types/crud-dialog-state.type';
  ...
  watch(()=> [dialogState.dialogState], async()=> {
    if (dialogState.dialogState === CrudDialogState.CREATE) {
      const response: any = await useApi('admin/stocking/stock-adjustment/next-code');
      if (response) {
